...
Preview documents, spreadsheets and slides in the browser before saving them. This default-off feature offers **Preview** and **Download** for supported downloads before Chrome asks for a save location. The dialog shows the filename and any size Chrome already knows. Document bytes are fetched, validated and cached only after you choose an action; dismissing an accidental prompt creates no document cache. A remembered action applies to all supported formats for the current website session. The contextual Document Preview button in the extension popup restores asking each time.

Supported formats: documents `.docx/.docm/.dotx/.dotm`; spreadsheets `.xlsx/.xlsm/.xltx/.xltm`; slides `.pptx/.pptm/.potx/.potm/.ppsx/.ppsm`; PDF `.pdf`. Settings also provides this list in a collapsed **Supported file formats** section.

Office previews preserve common text and table formatting, with worksheet and slide navigation where applicable. Word page breaks appear as separator lines. Zoom controls adjust Office content in 10% steps from 50% to 200%. Fonts use local fallbacks. These are content previews, not full Office applications: complex layouts, charts, SmartArt, animations and embedded media may be omitted or differ. Spreadsheet formulas show saved results when available; they are never recalculated. PDF files open in Chrome’s built-in viewer, with its own zoom and download controls. PDFs already open in that viewer retain their normal behavior.

Choose **Auto**, **Light** or **Dark** in settings. A sun/moon button switches the Office reading appearance for the current website session; **Default** restores the settings preference. Embedded pictures and original files remain unchanged. Chrome controls PDF page rendering, so the appearance setting changes the surrounding preview interface without recoloring the PDF pages. Turning off Document Preview stops new captures while keeping existing previews available until their session expires.

Office files are converted locally in a separate worker and displayed in a script-free sandbox with remote content blocked. Macro-enabled files expose static content only: macros, ActiveX controls and embedded programs do not run. PDF rendering relies on Chrome’s native viewer and browser sandbox. These measures reduce risk, but do not certify a file as safe; downloading keeps the original file, including any macros it contains.

Document caches live only in browser memory and are not uploaded to a conversion service. A cached file is cleared **3 hours after its last preview closes**, when the source website’s last tab closes or leaves, or when Chrome exits, whichever comes first. Reopening before expiry cancels the countdown while a preview remains open. Website choices and appearance overrides last for that website session across formats and subdomains, using eTLD+1; ordinary and incognito sessions are separate. Unsupported or uncorrelated downloads keep Chrome’s normal handling. A supported file that cannot be rendered can still be downloaded in its original form.
