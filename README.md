...
Word 97–2003 and binary PowerPoint files prioritize readable text and slide order rather than reproducing their layout. Rich Text and OpenDocument files retain supported basic formatting. Email previews show message headers and body, with attachment names only. Password-protected files are not supported. A loading line appears while the file is being prepared, without percentage numbers.
