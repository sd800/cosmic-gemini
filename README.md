...
Make bright text images easier to view when a page-wide dark mode is active on Xiaohongshu. XHS Image Dark Mode analyzes a reduced sample from each image, switches light text cards to dark, deepens uniform gray cards to a black background, and recognizes text layouts built from stable light and dark panels. Text cards with colored frames are supported, while photographs and mixed photo-and-text images remain unchanged. After opening a post, use the control beside its page count to switch one image, or press and hold to turn image adaptation off for that post and hide the control. The matching feed cover also returns to its original display, and reopening the post keeps it paused. User profile pages provide a separate switch that can pause analysis and adjustment for all posts on that profile.
