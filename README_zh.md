...
调暗小红书网页版在 Dark Reader 页面深色模式下仍然刺眼的文字图片。XHS Image Dark Mode 会分析每张图片的低分辨率样本，将浅色文字卡片切换为深色显示，并把均匀的灰底文字卡片进一步压暗为黑色背景。带有彩色边框的文字卡片也能识别，照片和图文混合图片则会保持原样。展开笔记后，您可以通过页码旁的按钮单独切换每张图片的浅色或深色显示。
