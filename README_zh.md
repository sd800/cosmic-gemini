...
Word 97–2003 和旧版 PowerPoint 文件以可读文字和幻灯片顺序为主，不力求还原原始排版。富文本与 OpenDocument 文件保留支持的基本格式。邮件预览显示邮件头与正文，附件仅显示名称。不支持受密码保护的文件。准备文件时会显示加载线条，不显示百分比。

Office 预览会保留常见的文字和表格格式，并提供工作表或幻灯片切换。Word 分页符以分隔线呈现，Office 内容缩放以 10% 为步长在 50% 至 200% 之间调整，字体使用本地替代字体。这是内容预览，并非完整的 Office 应用：复杂排版、图表、SmartArt、动画和嵌入媒体可能与原文件不同或无法显示。电子表格公式优先显示文件中已保存的结果，不会重新计算。PDF 使用 Chrome 内置阅读器，并保留其缩放和下载控件；已经在该阅读器中打开的 PDF 保持原有操作方式。
