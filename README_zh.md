...
调暗小红书网页版在页面深色模式下仍然刺眼的文字图片。XHS Image Dark Mode 会分析每张图片的低分辨率样本，将浅色文字卡片切换为深色显示，把均匀的灰底文字卡片进一步压暗为黑色背景，并识别由稳定浅色与深色区域组成的文字版式。带有彩色边框的文字卡片也能识别，照片和图文混合图片则会保持原样。展开笔记后，您可以通过页码旁的按钮单独切换当前图片，也可以长按暂停处理这篇笔记并隐藏按钮；瀑布流中的对应封面也会恢复原始显示，再次打开笔记时仍保持暂停。用户主页另有开关，可暂停识别和调整该主页中的全部笔记。
