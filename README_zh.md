...
在浏览器中预览文档、电子表格和演示文稿，再决定是否保存。此功能默认关闭，支持的下载会在 Chrome 询问保存位置前提供「预览」与「下载」选项。小浮窗显示文件名，Chrome 已知文件大小时也会一并显示。只有选择操作后，插件才会获取、验证并缓存文档；误触后关闭选择框不会留下文档缓存。为本次网站访问记住的操作适用于所有支持的格式，插件控制窗口中的动态文档按钮可恢复每次询问。

支持的格式：文档 `.docx/.docm/.dotx/.dotm`；电子表格 `.xlsx/.xlsm/.xltx/.xltm`；演示文稿 `.pptx/.pptm/.potx/.potm/.ppsx/.ppsm`；PDF `.pdf`。设置页默认折叠的「支持的文件格式」区域也会分类列出这些格式。
...
可在设置中选择「自动」「浅色」或「深色」。太阳／月亮按钮可切换同一网站本次访问中的 Office 阅读外观，「默认」按钮可恢复设置页的偏好。内嵌图片及原文件保持不变。PDF 页面的显示由 Chrome 管理，外观设置只调整周围的预览界面，不改变 PDF 页面颜色。关闭 Document Preview 只停止接管新的下载，已有预览仍保留至其会话过期。

Office 文件在本机的独立工作线程中转换，再由禁止脚本和远程内容的沙箱显示。含宏文件仅提供静态内容，不运行宏、ActiveX 控件或嵌入程序。PDF 使用 Chrome 原生阅读器及浏览器沙箱。这些措施用于降低风险，并不代表文件已通过安全认证；下载的仍是原文件，也会保留其中原有的宏。
