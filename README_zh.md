...
在保存前预览文档、电子表格、演示文稿、PDF 和邮件。此功能默认关闭；遇到支持的文件时，会在 Chrome 询问保存位置前提供「预览」或「下载」选项。您可以在本次网站访问中记住所选操作，也可以将网站加入白名单，保留正常下载流程。

支持的格式：文档 `.docx/.docm/.dotx/.dotm/.doc/.rtf/.odt`；电子表格 `.xlsx/.xlsm/.xltx/.xltm/.xls/.ods`；演示文稿 `.pptx/.pptm/.potx/.potm/.ppsx/.ppsm/.ppt/.odp`；PDF `.pdf`；其他 `.eml`。设置页也列有这份清单。

Office、富文本、OpenDocument 和邮件预览会保留常见格式，但复杂排版可能与原文件不同。受密码保护的 Office 文件暂不支持预览；支持的加密 PDF 可以输入密码后打开。

PDF Viewer 支持文字选择、搜索、翻页、缩略图、文件目录（如有）、缩放、旋转、打印及下载原文件。PDF 默认以 100% 打开。预览外观可设为「自动」「浅色」或「深色」，也可在本次网站访问中切换，不会修改原文件。

文件在本机处理。预览不会运行宏或加载远程内容。临时文件缓存在最后一个预览页关闭 10 分钟后、来源网站会话结束时，或 Chrome 退出时清理，以先到者为准。
...
在小红书网页启用页面深色模式时，调暗刺眼的文字卡片和聊天截图，保留照片原貌。瀑布流和展开的笔记都会自动调整；评论图片在笔记展开后处理。展开笔记后，可用图片按钮切换单张图片，长按可强制整篇笔记使用深色或浅色显示，单击则恢复自动识别。用户主页上的开关可暂停调整该主页的笔记；「始终启用」允许不检测页面深色模式而直接处理图片。
