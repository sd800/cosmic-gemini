...
预览默认跟随浏览器的深色或浅色外观，包括正文与表格。太阳／月亮按钮可切换当前预览的模式，「自动」按钮可恢复自动适配。内嵌图片及原始 DOCX 文件保持不变。关闭功能只停止接管新的下载，已有预览仍保留至来源网站的本次访问结束。

阅读版式会保留常见的字体、段落、列表和表格格式，显式分页符以分隔线呈现。缩放控件以 10% 为步长在 50% 至 200% 之间调整正文，点击百分比可恢复 100%。设备未安装的字体使用本地替代字体；预览不会完全复现 Word 的分页结果。

文字、表格和内嵌图片均在本地渲染，复杂排版可能与 Word 中有所不同，目前不支持其他 Office 格式。预览后仍可下载原文件。文档仅临时缓存在设备上，不会上传至转换服务。该网站的最后一个标签页关闭或离开网站后，缓存与本次操作偏好会被清除；网站按 eTLD+1 计算，各子域名共享本次访问状态。普通与无痕会话分别处理。无法可靠准备预览的文件仍按 Chrome 原有流程下载。
