...
在浏览器中预览 DOCX 文档，再决定是否保存。此功能默认关闭，支持的下载会在 Chrome 询问保存位置前提供「预览」与「下载」选项。小浮窗显示文件名，Chrome 已知文件大小时也会一并显示。只有选择「预览」或「下载」后，插件才会在本机获取、验证并缓存文档；误触后关闭选择框不会留下文档缓存。可为本次网站访问记住所选操作；插件控制窗口中的动态文档按钮可恢复每次询问。

可在 Document Preview 设置中选择「自动」「浅色」或「深色」，其中「自动」跟随浏览器外观。太阳／月亮按钮可切换同一网站（含子域名）的文档预览外观，直到该网站的最后一个标签页关闭或离开；「默认」按钮可恢复设置页中的偏好。正文与表格会随外观调整，内嵌图片及原始 DOCX 文件保持不变。关闭功能只停止接管新的下载，已有预览仍保留至来源网站的本次访问结束。

阅读版式会保留常见的字体、段落、列表和表格格式，显式分页符以分隔线呈现。缩放控件以 10% 为步长在 50% 至 200% 之间调整正文，点击百分比可恢复 100%。设备未安装的字体使用本地替代字体；预览不会完全复现 Word 的分页结果。

文字、表格和内嵌图片均在本地渲染，复杂排版可能与 Word 中有所不同，目前不支持其他 Office 格式。预览后仍可下载原文件。文档仅临时缓存在设备上，不会上传至转换服务。该网站的最后一个标签页关闭或离开网站后，缓存与本次操作偏好会被清除；每份文件的缓存也会在其最后一个预览页关闭 10 小时后清除，两者以先到者为准。到期前重新打开预览，会在预览仍打开时取消倒计时。网站按 eTLD+1 计算，各子域名共享本次访问状态。普通与无痕会话分别处理。无法可靠准备预览的文件仍按 Chrome 原有流程下载。
