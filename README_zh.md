...
保护您选中并复制的原文，阻止网站在复制时擅自夹带链接、署名或推广文字，并尽量保留原有格式。Clipboard Protect 不读取剪贴板中的原有数据，不记录或上传复制内容，默认关闭，可在 Satellites 中独立开启。富文本编辑器、在线表格等可编辑区域仍使用网站自身的复制方式；没有选中文字时，网页上的普通复制按钮也按原样工作。
