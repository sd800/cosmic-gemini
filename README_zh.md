...
控制窗口中的功能按钮只使用中性色和蓝色。一级功能未启用时会采用较清晰的中性色，强力模式等二级功能则使用更低调的灰度。蓝色表示该功能可以在当前标签页中工作。蓝色按钮带有背景时，表示功能已经开启，并可能继续影响页面显示或操作。Image Download 与 Video Download 的当前标签页会话开启后只显示蓝色，不添加背景。控制窗口中的 Cosmic Gemini 主标志保持不变，原有绿色工具栏图标仍保留给浏览器工具栏的独立活动提示。

### 网站规则

`example.com` 仅匹配该主机名。`*.example.com` 同时匹配根域名及其所有子域名。控制窗口会保存当前网站的精确规则，设置页面还可以添加通配规则。

Native Scroll 与 No Autoplay 各自拥有一个全局默认状态，以及四组网站规则：单独启用的网站、单独停用的网站、强力模式网站和标准模式网站。精确规则优先于通配规则，其次采用范围最具体的通配规则。范围相同时，单独停用优先于单独启用，标准模式优先于强力模式。因此，可以在较大范围的规则中继续设置更具体的例外。控制窗口中的一级功能按钮只会添加或移除当前网站的启用状态，不会改变全局默认设置。强力模式按钮会在需要时为当前网站启用功能，并在强力模式与标准模式之间切换。
