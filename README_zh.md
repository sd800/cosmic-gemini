...
可在设置中选择「自动」「浅色」或「深色」。太阳／月亮按钮可切换同一网站本次访问中的 非 PDF 阅读外观，「默认」按钮可恢复设置页的偏好。内嵌图片及原文件保持不变。PDF 页面的显示由 Chrome 管理，外观设置只调整周围的预览界面，不改变 PDF 页面颜色。关闭 Document Preview 只停止接管新的下载，已有预览仍保留至其会话过期。
