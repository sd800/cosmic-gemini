...
调暗小红书网页版在页面深色模式下仍然刺眼的文字图片。XHS Image Dark Mode 会分析图片的低分辨率样本，将浅色文字卡片切换为深色显示，把均匀的灰底文字卡片进一步压暗为黑色背景，并识别由稳定浅色与深色区域组成的文字版式。带有彩色边框的文字卡片也能识别，照片和图文混合图片则会保持原样。评论区图片只会在所属笔记展开后纳入处理，并在接近可视区域时识别。展开笔记后，长按页码旁的按钮可在整篇笔记强制深色与强制浅色之间切换，单击则恢复自动识别；瀑布流封面及稍后载入的图片也会遵循相同的整篇设置。用户主页另有开关，可暂停识别和调整该主页中的全部笔记。
