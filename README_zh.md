...
自行设置向网站提供的语言、地区格式、时区和隐私偏好信息。Website Knowledge Control 默认关闭。开启总开关后，可分别勾选要调整的项目：语言与 Intl 地区设置提供相同的地区选项，且均默认使用 `en-US`；时区需另行勾选，默认候选值为纽约，并遵循当地夏令时规则。Global Privacy Control 默认勾选，用于向网站表明请勿出售或共享您的个人数据。
