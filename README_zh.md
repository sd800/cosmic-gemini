...
文档缓存仅存于浏览器内存，不会上传至转换服务。缓存会在**最后一个预览页关闭 3 小时后**、来源网站的最后一个标签页关闭或离开时，或 Chrome 退出时清理，以先到者为准。到期前重新打开预览，会在预览仍打开时取消倒计时。操作与外观偏好在本次网站会话内跨格式、跨子域名生效，网站按 eTLD+1 计算；普通与无痕会话分别处理。不支持或无法可靠关联来源的下载保留 Chrome 原有流程，已接管但无法显示的文件仍可下载原文件。
