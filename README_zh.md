...
柔化网页中的明亮白色，包括白色字体、背景和图片。可选择三档米白色或冷蓝白。White Softer 默认关闭，所有显示调整均在本地完成。
...
在浏览器中预览文档、电子表格、演示文稿、PDF 和邮件，再决定是否保存。遇到支持的文件时，可先选择「预览」或「下载」。

支持 Word、Excel、PowerPoint、富文本、OpenDocument、PDF 和邮件格式。

预览会保留常见排版，但复杂内容可能与原文件有所不同。PDF 原本带有文字层时，还可选择和复制文字。文件在本机处理；预览不会运行宏或加载远程内容。
