...
Office 预览会保留常见的文字和表格格式，并提供工作表或幻灯片切换。Word 分页符以分隔线呈现，Office 内容缩放以 10% 为步长在 50% 至 200% 之间调整，优先使用浏览器可用的原字体；原字体不可用时，使用相近的本地字体，不下载字体。这是内容预览，并非完整的 Office 应用：复杂排版、图表、SmartArt、动画和嵌入媒体可能与原文件不同或无法显示。电子表格公式优先显示文件中已保存的结果，不会重新计算。PDF 使用 Chrome 内置阅读器，并保留其缩放和下载控件；已经在该阅读器中打开的 PDF 保持原有操作方式。

可在设置中选择「自动」「浅色」或「深色」。太阳／月亮按钮可切换同一网站本次访问中的阅读外观，「默认」按钮可恢复设置页的偏好。PDF 深色模式采用 Dark Reader 式反色与对比度调整，继续使用 Chrome 内置阅读器；PDF 中的图片、阅读器控件及纸张外侧背景也会随之变色。非 PDF 文档的内嵌图片及所有原文件保持不变。关闭 Document Preview 只停止接管新的下载，已有预览仍保留至其会话过期。
