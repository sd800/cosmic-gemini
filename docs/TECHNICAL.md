...
The main-world runtime neutralizes matching `fetch`, XHR, Beacon, and tracking-image requests before they reach the network, while Chrome's session DNR rules provide an early fallback for scripts, requests, frames, images, and the Tencent loopback probe. The Tencent News policy hides a narrow set of confirmed advertising containers with one local style. No policy removes framework-managed nodes or changes Beacon metadata. The runtime uses no observer or polling loop and restores every patched API, style, blob URL, bridge, runtime, and temporary network rule when the product is disabled.
