(() => {
  const BRIDGE_KEY = Symbol.for('cosmic-gemini.chinese-punctuation-claude.bridge');
  ...
  const requestConfig = async () => {
    ...
    try {
      const response = await chrome.runtime.sendMessage({ type: 'CG_PAGE_STATE', featureId: 'chinesePunctuationClaude' });
      if (disposed) return;
      if (request !== configRequest) return false;
      const config = response?.result?.chinesePunctuationClaude;
  ...
  function onMessage(message, _sender, sendResponse) {
    if (message?.type === 'CG_STOP_CENTRAL_FEATURE' && message.featureId === 'chinesePunctuationClaude') {
      dispose();
      sendResponse({ disposed: true });
    } else if (message?.type === 'CG_REFRESH_FEATURE_CONFIG' && message.featureId === 'chinesePunctuationClaude') {
