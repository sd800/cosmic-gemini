(() => {
  const BRIDGE_KEY = Symbol.for('cosmic-gemini.clipboard-protect.bridge');
  if (globalThis[BRIDGE_KEY]) return;
  const READY = 'cosmic-gemini:clipboard-protect:bridge-ready';
  const MAIN_READY = 'cosmic-gemini:clipboard-protect:main-ready';
  const CONFIGURE = 'cosmic-gemini:clipboard-protect:configure';
  const DISPOSE = 'cosmic-gemini:clipboard-protect:dispose';
  let token = '';
  let disposed = false;
  ...
  const requestConfig = async () => {
    if (disposed) return;
    try {
      const response = await sendRuntimeMessage({ type: 'CG_PAGE_STATE', featureId: 'clipboardProtect' });
      if (disposed) return;
      const config = response?.result?.clipboardProtect;
  ...
  function onMessage(message, _sender, sendResponse) {
    if (message?.type === 'CG_STOP_CENTRAL_FEATURE' && message.featureId === 'clipboardProtect') {
      dispose();
      sendResponse({ disposed: true });
    } else if (message?.type === 'CG_REFRESH_FEATURE_CONFIG' && message.featureId === 'clipboardProtect') {
      void requestConfig();
    }
  }
