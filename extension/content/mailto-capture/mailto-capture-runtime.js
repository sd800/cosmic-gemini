(() => {
  ...
  const PHONE_KEY = Symbol.for('cosmic-gemini.mailto-capture.phone');
  ...
  const protocols = globalThis[Symbol.for('cosmic.external-links-capture.protocols')];
  ...
  class MailtoCaptureRuntime {
    ...
    onDispose(event) {
      ...
      try { delete globalThis[PHONE_KEY]; } catch {}
    }
    ...
    parseMailto(href) { return protocols.parseMailto(href); }

    parseTel(href) {
      const parsed = protocols.parseTel(href);
      return parsed ? { ...parsed, location: telephoneLocation(parsed.number, this.locale) } : null;
    }

    parseSms(href) {
      const parsed = protocols.parseSms(href);
      return parsed ? { ...parsed, locations: parsed.recipients
        .map(number => ({ number, location: telephoneLocation(number, this.locale) }))
        .filter(item => item.location) } : null;
    }

    parseLink(href) { return this.parseMailto(href) || this.parseTel(href) || this.parseSms(href); }
    ...
    displayTelephoneNumber(value) {
      ...
      try {
        const display = globalThis[PHONE_KEY]?.inspect(raw)?.display;
    ...
    appendTelephoneField(parent, labelText, entries) {
      ...
      for (const item of entries) {
        ...
        if (item.location) {
          ...
          this.appendLocationText(location, item.location);
    ...
    appendLocationText(element, value) {
      ...
      if (numberingPlanAt >= 0) {
        ...
        if (regionAt < 0) tail.textContent = text;
          ...
          element.textContent = text.slice(0, regionAt + 2);
          tail.textContent = text.slice(regionAt + 2);
        }
      }
      element.append(tail);
    }
