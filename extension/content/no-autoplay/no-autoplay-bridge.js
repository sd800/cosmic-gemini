(() => {
  ...
  const requestConfig = async () => {
    if (disposed) return;
    try {
      const response = await sendRuntimeMessage({ type: 'CG_PAGE_STATE', featureId: 'noAutoplay' });
      if (disposed) return;
      ...
      void sendRuntimeMessage({ type: 'CG_CONFIG_APPLIED', featureId: 'noAutoplay', active: true }).catch(() => {});
    } catch {
      if (disposed) return;
      configFailures += 1;
  ...
  function onIntervened(event) {
    ...
    void sendRuntimeMessage({ type: 'CG_FEATURE_INTERVENED', featureId: 'noAutoplay', pageUrl: location.href }).catch(() => {});
  }
  function onMessage(message, _sender, sendResponse) {
    if (message?.type === 'CG_STOP_CENTRAL_FEATURE' && message.featureId === 'noAutoplay') {
      dispose();
