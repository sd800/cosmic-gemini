...
function canonicalHostname(value) {
  ...
  if ((/^[\x00-\x7f]+$/.test(raw) && hostname !== raw) || hostname.length > 253 || !hostname.includes('.') || hostname.split('.').some(label => !HOST_LABEL.test(label))) {
...
export function normalizeSettings(value = {}) {
  ...
  return {
    ...
    pageDisplay: {
      ...
      greyscale: {
        enabled: value.pageDisplay?.greyscale?.enabled === true
      }
    },
    xhsImageDarkMode: {
