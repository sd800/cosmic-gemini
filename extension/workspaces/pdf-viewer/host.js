...
export function createPdfViewer({ container, bytes, filename, locale, dark, automatic, onDownload, onTheme, onAuto, onError }) {
  ...
  iframe.addEventListener('load', () => { if (!closed) iframe.contentWindow.postMessage({ type: 'CG_PDF_INIT', bytes, filename, locale, dark, automatic }, '*', [channel.port2, bytes]); }, { once: true });
