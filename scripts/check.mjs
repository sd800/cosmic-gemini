...
assert.equal(manifest.version, '7.7.1');
...
assert.match(satellitesSettings, /id="mailtoCaptureEnabled"[\s\S]*id="reduceWhitePointEnabled"[\s\S]*id="xhsImageDarkModeEnabled"[\s\S]*id="biliDailyLogin"/);
...
assert.match(settingsSource, /featureId: 'reduceWhitePoint'[\s\S]*UI_SET_REDUCE_WHITE_POINT_SETTING/);
assert.match(settingsPreload, /reduceWhitePointEnabled[\s\S]*reduceWhitePointReduction/);
...
const reduceWhitePoint = await source('background', 'products', 'operations', 'reduce-white-point.js');
const reduceWhitePointBridge = await source('content', 'reduce-white-point-bridge.js');
const reduceWhitePointRuntime = await source('content', 'reduce-white-point-runtime.js');
...
assert.match(operations, /createAnyCopyProduct[\s\S]*createAnyCopyEnhancedProduct[\s\S]*createSatellitesProduct[\s\S]*createReduceWhitePointProduct[\s\S]*createXhsImageDarkModeProduct[\s\S]*createAdministrationProduct/);
...
assert.match(reduceWhitePoint, /content\/reduce-white-point-bridge\.js[\s\S]*content\/reduce-white-point-runtime\.js/);
assert.match(reduceWhitePoint, /context\.frameId === 0[\s\S]*pageRuntimeHost\.sync/);
assert.match(reduceWhitePointBridge, /CG_PAGE_STATE'[\s\S]*featureId: 'reduceWhitePoint'/);
assert.match(reduceWhitePointBridge, /sendResponse\(\{ disposed: true \}\)/);
assert.doesNotMatch(reduceWhitePointBridge, /chrome\.storage/);
assert.match(reduceWhitePointRuntime, /attachShadow\(\{ mode: 'closed'/);
assert.match(reduceWhitePointRuntime, /position:fixed[\s\S]*inset:0[\s\S]*pointer-events:none/);
assert.match(reduceWhitePointRuntime, /document\.fullscreenElement[\s\S]*fullscreenchange/);
assert.doesNotMatch(reduceWhitePointRuntime, /MutationObserver|IntersectionObserver|ResizeObserver|setInterval|fetch\s*\(|XMLHttpRequest|WebSocket|addEventListener\(['"](?:click|pointer|wheel|touch|key)/);
...
assert.match(config, /reduceWhitePoint:[\s\S]*enabled: false[\s\S]*reduction: 0\.25/);
...
assert.doesNotMatch(centralPage, /nativeScroll|noAutoplay|mailtoCapture|adMarshal|anyCopy|reduceWhitePoint|imageDownload|videoDownload|chrome\.storage/);
