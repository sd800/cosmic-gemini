...
module.exports = {...mammoth,
  ...
  const partPath=(type,fallback)=>{const target=rels?.children?.find(node=>node.attributes?.Type?.endsWith('/'+type)&&node.attributes.TargetMode!=='External')?.attributes.Target;return target&&!/^[a-z]+:|^\/|\\|(?:^|\/)\.\.(?:\/|$)/i.test(target)?directory+target:directory+fallback;};
  const parts=Object.fromEntries(await Promise.all(Object.entries({styles:paths.styles,numbering:paths.numbering,document:paths.mainDocument,theme:partPath('theme','theme/theme1.xml'),settings:partPath('settings','settings.xml')}).map(async([key,path])=>[key,await officeXml.readXmlFromZipFile(zip,path)])));
