...
async function opendocument(buffer,format){
  ...
  const registered=styles(),definitions=new Map(),defaults=new Map(),cache=new Map();let output=0,nodes=0;
  for(const root of [styleRoot,content]){
    for(const node of all(root,'style:style'))definitions.set(attr(node,'style:name'),node);
  ...
  function properties(node){const result={};for(const n of node?.children||[]){
    for(const [from,to] of [['fo:font-size','font-size'],['fo:margin-top','margin-top'],['fo:margin-bottom','margin-bottom'],['fo:margin-left','margin-left'],['fo:margin-right','margin-right'],['fo:text-indent','text-indent'],['fo:padding','padding-left'],['style:column-width','width'],['style:row-height','height']])if(length(attr(n,from)))result[to]=length(attr(n,from));
    ...
    if(attr(n,'fo:font-weight')==='bold')result['font-weight']='700';
    if(attr(n,'fo:font-style')==='italic')result['font-style']='italic';
    ...
    const decoration=[];if(attr(n,'style:text-underline-style')&&attr(n,'style:text-underline-style')!=='none')decoration.push('underline');if(attr(n,'style:text-line-through-style')&&attr(n,'style:text-line-through-style')!=='none')decoration.push('line-through');if(decoration.length)result['text-decoration-line']=decoration.join(' ');
  }return result;}
  function resolve(name,family,seen=new Set()){
    ...
    const node=definitions.get(name),parent=attr(node,'style:parent-style-name');
  ...
  async function render(node,depth=0){
    ...
    const inner=(await Promise.all((node.children||[]).map(child=>render(child,depth+1)))).join('');
    ...
    else if(name==='text:list')tag='ul';else if(name==='text:list-item')tag='li';
    else if(name==='text:a')return '<a href="'+esc(attr(node,'xlink:href'))+'">'+inner+'</a>';
    else if(name==='draw:frame')tag='div';
    if(!tag)return inner;
    const definition=definitions.get(attr(node,'text:style-name')),breakBefore=attr(first(definition,'style:paragraph-properties'),'fo:break-before')==='page';
  ...
  async function renderTable(table,sheet){
    ...
    for(const row of rows){
      ...
      for(const cell of row.children||[]){if(!['table:table-cell','table:covered-table-cell'].includes(cell.name))continue;
        ...
        const visible=(cell.children||[]).some(c=>c.type==='element'&&c.name!=='office:annotation')||attr(cell,'office:value')!==''||attr(cell,'office:string-value')!==''||attr(cell,'table:formula')!=='';
    ...
    for(const {row,values,position,repeat} of parsed){if(position>lastContentRow)break;
      ...
      for(const {cell,col,n,visible} of values){if(col>maxCol)break;
        ...
        let value='';if(visible)value=(await Promise.all((cell.children||[]).map(c=>render(c)))).join('');
  ...
  if(format==='odp'){
    ...
    for(const [i,page] of pages.entries()){
      const master=all(styleRoot,'style:master-page').find(n=>attr(n,'style:name')===attr(page,'draw:master-page-name'));
      const layout=all(styleRoot,'style:page-layout').find(n=>attr(n,'style:name')===attr(master,'style:page-layout-name'));
      const props=first(layout,'style:page-layout-properties');
      const slide=registered.add({width:length(attr(props,'fo:page-width'))||'720pt',height:length(attr(props,'fo:page-height'))||'540pt','background-color':'#ffffff'});
  ...
  return {value,formatting:{styles:registered.values}};
}
