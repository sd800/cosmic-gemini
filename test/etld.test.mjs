...
test('etld contains the complete ICANN snapshot and only the selected PRIVATE exceptions', () => {
  ...
  assert.deepEqual(etldMetadata.geographicPrivateTlds, ['krd', 'scot']);

  for (const rule of ['com', 'co.uk', '*.ck', '!www.ck', 'scot', 'krd']) {
  ...
  for (const rule of [
    ...
    'co.krd', 'edu.krd', 'co.scot', 'me.scot', 'org.scot',
    'gov.scot', 'service.gov.scot', 'mygov.scot'
  ]) {
